...
**Status:** Recommended for Stage 0 approval subject to the stated gates  
**Version:** 1.0  
**Date:** 3 August 2026  
...
This foundation defines six stages:
...
Stages 1–5 form the research platform. Stage 6 is a deliberately separated downstream possibility, not an implementation instruction and not part of Stage 0.
...
- robustness and evidence-status rules; and
...
**MDM snapshot → admissibility checks → point-in-time observations → versioned features → exploratory discovery → candidate record → preregistered validation → independent reimplementation → direct replication → evidence synthesis**
...
| Knowledge curation | Synthesize all evidence and contradictions | Store only successful findings |
...
3. preregistration;
4. authorised MDM snapshot identity;
5. eligible-universe definition;
6. feature-definition versions;
7. immutable execution record;
8. complete statistical output, including diagnostics;
9. validation decision;
10. independent-reimplementation and direct-replication records;
11. evidence synthesis; and
12. knowledge-base status history.
...
The feature engine creates hundreds of deterministic measurements for every eligible security and trading day, subject to MDM coverage. Its goal is rich, auditable measurement—not indiscriminate indicator generation.
...
| Experiment feature view | Frozen selection of feature versions tied to a preregistration |
...
- row counts, coverage diagnostics, and fingerprints; and
...
1. **Eligibility gate:** The complete Stage 3 record, search lineage, and candidate definition are available.
...
4. **Primary-test gate:** The prespecified estimand and inference method are run once on the reserved sample.
5. **Temporal gate:** The unchanged phenomenon is assessed through registered rolling-origin or walk-forward evidence; descriptive monitoring is kept separate.
6. **Cross-sectional gate:** Mechanism-led confirmatory interactions and a standard descriptive transportability panel examine capitalisation, sector, industry, liquidity, and lifecycle heterogeneity.
7. **Regime gate:** Preregistered regime claims are tested inferentially; other bull/bear, volatility, participation, liquidity, and related states are descriptive diagnostics.
8. **Robustness and falsification gate:** Negative controls and adversarial sensitivity checks challenge the claim.
9. **Multiplicity gate:** Global ancestry is disclosed and the applicable discovery, sealed-sample, and sequential decision families pass their registered error control.
10. **Reproduction and replication gate:** The result is computationally reproduced, independently reimplemented, and directly tested on genuinely new eligible outcomes.
11. **Adjudication gate:** A governed decision and complete dossier enter the Knowledge Base regardless of outcome.
...
- feature versions and transformations;
...
- start, finish, executor, and execution provenance;
...
The Knowledge Base must support search by construct, exposure, outcome, horizon, population, data field, feature family, sector, market-cap group, regime, mechanism, evidence state, and failure reason.
...
- competing explanations already examined; and
...
10. no optimisation occurs before validated discovery; and
...
| Knowledge custodian | Maintains statuses, links, negative records, and revalidation | Cannot delete adverse evidence |
...
**propose → feasibility review → preregister → freeze → execute → deposit all results → computationally reproduce → provisionally adjudicate → independently reimplement → directly replicate → synthesize → assign governed phenomenon maturity → revalidate**
...
- holdout and replication policy; and
...
Name the data steward, methods reviewer, knowledge custodian, and replication lead. Approve conflict and role-overlap disclosures. No phenomenon should be promoted by its sole author.
...
- test families and research budgets are assigned;
...
5. **Scientific/strategy boundary erosion.** Performance-led thresholds can quietly turn a phenomena programme into an optimiser.
...
- the append-only Knowledge Base and negative-results model are accepted;
- review roles and decision rights are assigned;
- each critical risk is closed or accepted by a named accountable owner; and
...
- reviewed for data feasibility and method; and
- allocated valid discovery and reserved evidence.

### 17.3 Stage 2 feature-readiness gate

A feature is experiment-ready only when:
...
1. **Registration:** Protocol is complete and frozen before relevant outcome access.
2. **Data integrity:** MDM lineage, universe, identity, timing, and corporate actions pass.
3. **Execution integrity:** All registered analyses complete; deviations and failed checks are deposited.
4. **Interpretation:** The governed estimator row, registered decision function, selection-aware multiplicity, magnitude, uncertainty, and dependence-aware power are handled correctly.
5. **Confirmation:** The claim passes one genuinely locked sample.
6. **Replication:** Independent reimplementation and direct replication on genuinely new outcomes meet a prediction, compatibility, or meta-analytic criterion registered in advance.
7. **Knowledge admission:** Supported, precise-null, contradicted, inconclusive, and invalid conclusions are curated with limitations.
8. **Revalidation:** Current status is maintained only while new evidence remains within declared bounds.

No universal p-value or single score determines passage. Leakage, invalid point-in-time construction, concealed search, and non-reproducibility are absolute failures.
...
- Evidence-card and revalidation updates
...
The central scientific risks are point-in-time leakage, false precision from dependent equity panels, multiplicity across a large feature space, holdout exhaustion, microstructure and small-cap artifacts, post-hoc explanation, and gradual erosion of the phenomenon/strategy boundary. The principal omissions are the unresolved market scope and the unverified extent of MDM’s point-in-time coverage.

Subject to those explicit Stage 0 gates, this architecture is recommended as the scientific foundation for Stage 1. It supports broad discovery without rewarding indiscriminate search, preserves failure as knowledge, gives cross-sectional equity context first-class status, and keeps any future strategy activity downstream of independently validated science.
